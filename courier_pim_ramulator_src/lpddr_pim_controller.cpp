...
class LPDDRPIMController final : public IDRAMController, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAMController, LPDDRPIMController, "LPDDR-PIM", "A LPDDR-PIM controller.");
  private:
  ...
  public:
    ...
    bool send(Request& req) override {
      ...
      switch (req.type_id) {
        ...
        case Request::Type::PIM_ACC:        is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_AF:         is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_EWMUL:      is_success = m_pim_buffer.enqueue(req);   break;
    ...
    void tick() override {
      ...
      // 与原ramulator差异点，没有这个调度优化策略，这里假设行列命令可以并发发出
  ...
  private:
    ...
    bool schedule_request(ReqBuffer::iterator& req_it, ReqBuffer*& req_buffer) {
      ...
      // 这段代码的逻辑和原ramulator相似，都是防止关闭正在使用的行，但具体实现方法有点不同
    ...
    // 后面这些函数都是为了支持行列命令并发调度而设置的（似乎是HBM3才有的），如果去掉这个功能，可以把这些函数删掉
    /**
    ...
    int get_command_type(int command) {
      if      (command == m_dram->m_commands("ACT-1"))   return 0; // Row command
      else if (command == m_dram->m_commands("ACT-2"))   return 0; // Row command
      ...
      else if (command == m_dram->m_commands("ACC"))   return 1; // Col command
      else if (command == m_dram->m_commands("AF"))    return 1; // Col command
      else if (command == m_dram->m_commands("EWMUL")) return 1; // Col command
      else return -1;
    }
