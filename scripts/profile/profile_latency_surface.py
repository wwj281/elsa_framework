...
Usage:
    ...
    python scripts/profile/profile_latency_surface.py --model Mixtral-8x7B --list_data_dirs

    # Quick test mode
    python scripts/profile/profile_latency_surface.py --model Mixtral-8x7B --quick_test

    # Default profiling
    python scripts/profile/profile_latency_surface.py --model Mixtral-8x7B

    # Full grid profiling
    python scripts/profile/profile_latency_surface.py --model Mixtral-8x7B --full_grid

    # With Phase 1 feasible region file
    python scripts/profile/profile_latency_surface.py \
        --model Mixtral-8x7B \
        --feasible_region_file configs/feasible_region/deepseek_16b.json \
        --output_lut results/dwap_phase2/latency_lut.json
"""
...
DWAP_RESULT_ROOT = PROJECT_ROOT / "results" / "dwap_phase2"
FEASIBLE_REGION_ROOT = PROJECT_ROOT / "configs" / "feasible_region"
...
def data_path(relative_path: str) -> Path:
    ...
    if path.parts and path.parts[0] == "gate_weight_data":
        return GATE_WEIGHT_ROOT / Path(*path.parts[1:])
    return GATE_WEIGHT_ROOT / path
...
def resolve_input_path(path_value: str) -> Path:
    ...
    if path.is_absolute():
        return path
    if path.parts and path.parts[0] == "feasible_region":
        return FEASIBLE_REGION_ROOT / Path(*path.parts[1:])
...
def list_available_data_dirs(model_name: str) -> List[Dict]:
    ...
    gate_weight_base = GATE_WEIGHT_ROOT
    available = []
...
def get_data_dir(model_name: str, seq_length: int, batch_size: int) -> str:
    ...
    full_path = data_path(exact_path)
...
class LatencySurfaceProfiler:
    ...
    def _create_system(self,
        ...
        """
        # Model configuration
        moe = self.model_name in ['DeepSeek-16B', 'Qwen-2.7B', 'Mixtral-8x7B', 'Qwen-3-30B', 'GPT-OSS-120B']
        ...
        # Data file paths
        tfs_path = str(data_path(tfs_file))
        print('tfs_path:', tfs_path)
        gss_path = str(data_path(gss_file))
        print('gss_path:', gss_path)
        elp_path = str(data_path(elp_file))
...
def parse_args():
    ...
    parser.add_argument(
        "--model", type=str, default="DeepSeek-16B",
    ...
    # Input/Output files
    parser.add_argument(
        "--feasible_region_file", type=str, default="configs/feasible_region/deepseek_16b.json",
        help="Path to feasible region JSON from Phase 1 (optional)"
    )
    parser.add_argument(
        "--output_lut", type=str, default="results/dwap_phase2/latency_lut.json",
        help="Output path for the Look-Up Table JSON"
    )
    parser.add_argument(
        "--output_csv", type=str, default="results/dwap_phase2/latency_profiling_detailed.csv",
...
def main():
    ...
    if args.list_data_dirs:
        ...
        if available:
            ...
            print(f"  Expected pattern: data/gate_weight/input{{seq}}_batch{{batch}}/{{model_dir}}")
        return 0
    ...
    # 加载可行域
    if args.feasible_region_file:
        feasible_region_path = resolve_input_path(args.feasible_region_file)
        if os.path.exists(feasible_region_path):
            feasible_region = profiler.load_feasible_region(str(feasible_region_path))
            print(f"[Info] Loaded feasible region from {feasible_region_path}: {len(feasible_region)} points")
        else:
            print(f"[Error] feasible_region_file not found: {feasible_region_path}")
            print("[Hint] Check files under configs/feasible_region/")
            return 1
    else:
        feasible_region = profiler._generate_default_feasible_region()
        print(f"[Info] Using default feasible region: {len(feasible_region)} points")

    # 配置运行模式
    if args.quick_test:
        print("[Info] Quick test mode: minimal configuration")
        # If feasible_region was loaded from file, keep it and only reduce point count.
        # Otherwise keep the default quick-test pair.
        if feasible_region:
            feasible_region = feasible_region[: min(2, len(feasible_region))]
        else:
        ...
        # 默认: 中等规模配置
        profiler.batch_buckets = [(1, 3), (4, 8), (9, 16)]
    ...
    # 保存结果
    profiler.save_lut(lut, str(resolve_output_path(args.output_lut)))
    profiler.save_detailed_csv(str(resolve_output_path(args.output_csv)))
