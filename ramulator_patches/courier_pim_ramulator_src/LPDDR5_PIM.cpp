...
public:
  ...
  inline static const ImplLUT m_command_scopes = LUT(
      m_commands, m_levels, {{"ACT-1", "row"}, {"ACT-2", "row"}, {"PRE", "bank"}, {"PREA", "dimm"}, {"CASRD", "dimm"}, {"CASWR", "dimm"}, {"RD", "column"}, {"WR", "column"}, {"RDA", "column"}, {"WRA", "column"}, {"REFab", "dimm"}, {"REFpb", "dimm"}, {"RFMab", "dimm"}, {"RFMpb", "dimm"},
                             // PIM commadns
                             {"ACTAB", "row"},
                             {"ACTSB", "row"},
                             {"ACTPB", "row"},
                             {"MACAB", "column"},
                             {"MACSB", "column"},
                             {"MACPB", "column"}, // ACTPB and MACPB are broadcasted to pCHs in a channel
                             {"WRGB", "dimm"},
                             {"MVSB", "bank"},
                             {"MVGB", "bank"},
                             {"SFM", "channel"},
  ...
  inline static constexpr ImplDef m_requests = {
      ...
      "pim-accumulate", "pim-activation-function", "pim-elementwise-multiply", "pim-barrier", "per-bank-refresh"};

  inline static const ImplLUT m_request_translations = LUT(
      m_requests, m_commands, {{"read16", "RD"}, {"write16", "WR"}, {"all-bank-refresh", "REFab"}, {"per-bank-refresh", "REFpb"},
                               // PIM requests
  ...
  private:
    ...
    void set_timing_vals() {
      ...
      populate_timingcons(this, {
          ...
          {.level = "channel", .preceding = {"ACTAB"}, .following = {"ACTAB", "ACT-2", "PRE", "PREA", "REFab"}, .latency = 2},
          /// All banks in a dimm 
          ...
          /*** Channel ***/ 
          {.level = "channel", .preceding = {"WRGB"}, .following = {"WRGB"}, .latency = V("nBL16")},
          ...
          // CAS <-> CAS
          /// Data bus occupancy
          {.level = "channel", .preceding = {"RD", "RDA"}, .following = {"RD", "RDA"}, .latency = V("nBL16")},
          {.level = "channel", .preceding = {"WR", "WRA"}, .following = {"WR", "WRA"}, .latency = V("nBL16")},
          ...
          {.level = "rank", .preceding = {"RD", "RDA"}, .following = {"WR", "WRA"}, .latency = V("nCL") + V("nBL16") + 2 - V("nCWL")},
          /// WR <-> RD, Minimum Read after Write
          {.level = "rank", .preceding = {"WR", "WRA"}, .following = {"RD", "RDA"}, .latency = V("nCWL") + V("nBL16") + V("nWTRS")},
          /// CAS <-> CAS between sibling ranks, nCS (rank switching) is needed for new DQS
          {.level = "rank", .preceding = {"RD", "RDA"}, .following = {"RD", "RDA", "WR", "WRA"}, .latency = V("nBL16") + V("nCS"), .is_sibling = true},
          {.level = "rank", .preceding = {"WR", "WRA"}, .following = {"RD", "RDA"}, .latency = V("nCL")  + V("nBL16") + V("nCS") - V("nCWL"), .is_sibling = true},
          /// CAS <-> PREab
          {.level = "rank", .preceding = {"RD"}, .following = {"PREA"}, .latency = V("nRTP")},
          {.level = "rank", .preceding = {"WR"}, .following = {"PREA"}, .latency = V("nCWL") + V("nBL16") + V("nWR")},          
          /// RAS <-> RAS
          ...
          {.level = "rank", .preceding = {"WRA"}, .following = {"REFab"}, .latency = V("nCWL") + V("nBL16") + V("nWR") + V("nRPpb")},          
          ...
          /*** Same Bank Group ***/ 
          /// CAS <-> CAS
          {.level = "bankgroup", .preceding = {"RD", "RDA"}, .following = {"RD", "RDA"}, .latency = V("nCCD")},          
          {.level = "bankgroup", .preceding = {"WR", "WRA"}, .following = {"WR", "WRA"}, .latency = V("nCCD")},          
          {.level = "bankgroup", .preceding = {"WR", "WRA"}, .following = {"RD", "RDA"}, .latency = V("nCWL") + V("nBL16") + V("nWTRL")},
          /// RAS <-> RAS
          ...
          {.level = "bank", .preceding = {"WR"},  .following = {"PRE"}, .latency = V("nCWL") + V("nBL16") + V("nWR")},  
          {.level = "bank", .preceding = {"RDA"}, .following = {"ACT-1"}, .latency = V("nRTP") + V("nRPpb")},  
          {.level = "bank", .preceding = {"WRA"}, .following = {"ACT-1"}, .latency = V("nCWL") + V("nBL16") + V("nWR") + V("nRPpb")},  
        }
      );
      #undef V
    ...
    void set_actions() {
      ...
      m_actions[m_levels["rank"]][m_commands["RD"]] = [] (Node* node, int cmd, int target_id, Clk_t clk) {
        node->m_final_synced_cycle = clk + m_timings["nCL"] + m_timings["nBL16"]; 
      };
      m_actions[m_levels["rank"]][m_commands["WR"]] = [] (Node* node, int cmd, int target_id, Clk_t clk) {
      ...
      m_actions[m_levels["bank"]][m_commands["RDA"]] = Lambdas::Action::Bank::PRE<LPDDR5PIM>;
      m_actions[m_levels["bank"]][m_commands["WRA"]] = Lambdas::Action::Bank::PRE<LPDDR5PIM>;
    };
    ...
    void set_preqs() {
      ...
      // Bank Preqs
      m_preqs[m_levels["bank"]][m_commands["RD"]] = [] (Node* node, int cmd, int target_id, Clk_t clk) {
        switch (node->m_state) {
      ...
      m_preqs[m_levels["bank"]][m_commands["WR"]] = [] (Node* node, int cmd, int target_id, Clk_t clk) {
        switch (node->m_state) {
          case m_states["Closed"]: return m_commands["ACT-1"];
          case m_states["Pre-Opened"]: return m_commands["ACT-2"];
          case m_states["Opened"]: {
    ...
    void set_rowhits() {
      ...
      m_rowhits[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowHit::Bank::RDWR<LPDDR5PIM>;
      m_rowhits[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowHit::Bank::RDWR<LPDDR5PIM>;
    ...
    void set_rowopens() {
      ...
      m_rowopens[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowOpen::Bank::RDWR<LPDDR5PIM>;
      m_rowopens[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowOpen::Bank::RDWR<LPDDR5PIM>;
