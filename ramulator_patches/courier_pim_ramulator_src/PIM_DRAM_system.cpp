...
class PIMDRAMSystem final : public IMemorySystem, public Implementation {
  ...
  public:
    ...
    int s_num_pim_accumulate_requests = 0;
    int s_num_pim_activate_function_requests = 0;
    int s_num_pim_multiply_element_wise_requests = 0;
  ...
  public:
    void init() override { 
      ...
      register_stat(s_num_pim_accumulate_requests).name("total_num_pim_accumulate_requests");
      register_stat(s_num_pim_activate_function_requests).name("total_num_pim_activate_function_requests");
      register_stat(s_num_pim_multiply_element_wise_requests).name("total_num_pim_multiply_element_wise_requests");
    ...
    bool send(Request req) override {
      ...
      if (is_success) {
        switch (req.type_id) {
          ...
          case Request::Type::PIM_ACC: {
            s_num_pim_accumulate_requests++;
            break;
          }
          case Request::Type::PIM_AF: {
            s_num_pim_activate_function_requests++;
            break;
          }
          case Request::Type::PIM_EWMUL: {
            s_num_pim_multiply_element_wise_requests++;
            break;
          }
