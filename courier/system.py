...
RAMPATH = str(PROJECT_ROOT / "ramulator2")
RAMLOG = str(PROJECT_ROOT / "results" / "raw_logs" / "ramulator.out")
...
class System:
    ...
    def set_accelerator(self, modelinfos, name: DeviceType, config):
        ...
        if self.hetero_name == DeviceType.PIM:
            COURIER_LOG_DIR.mkdir(parents=True, exist_ok=True)
            log_name = f"{config['PIM_TYPE'].name}_C{config['NUM_HBM']}_M{config['MAPPING_STRATEGY'].name}.log"
            output_log = str(COURIER_LOG_DIR / log_name)
            self.mapping_strategy = config['MAPPING_STRATEGY']
            ramulator = Ramulator(modelinfos, ramulator_dir=RAMPATH, pim_type=config['PIM_TYPE'],
    ...
    def simulate(self,
        ...
        print(f"Expert schedule simulation time: {end_time - start_time:.9f} s")
        ...
        print(f"gpu_total_time: {expert_schedule['gpu_total_time']:.9f} s")
        print(f"     c_time: {expert_schedule['gpu_latency_breakdown']['compute']:.9f} s")
        print(f"     m_time: {expert_schedule['gpu_latency_breakdown']['mem']:.9f} s")
        print(f"     l_time: {expert_schedule['gpu_latency_breakdown']['load']:.9f} s")
        print(f"acc_total_time: {expert_schedule['acc_total_time']:.9f} s")
        print(f"     c_time: {expert_schedule['acc_latency_breakdown']['compute']:9f} s")
        print(f"     m_time: {expert_schedule['acc_latency_breakdown']['mem']:9f} s")
        print(f"     l_time: {expert_schedule['acc_latency_breakdown']['load']:9f} s")
        print(f"total_latency: {expert_schedule['total_latency']:.9f} s")
                ...
                if layer.type == LayerType.X2G:
                    ...
                    if move_to_gpu_ids:
                        ...
                        layer.energy = [0, 0, 0, 0, 0, 0]
                                                                                          ...
                                                                                          DeviceType.PIM,DeviceType.NONE]:
                    if layer.name == 'ff1':
                        ...
                        # 分别计算加速器专家
                        if schedule_strategy == ScheduleStrategyType.PIMOE and 'channel_experts' in expert_schedule:
                            channel_exec_times = {}
                            for channel_id, channel_eids in expert_schedule['channel_experts'].items():
                                channel_time = 0.0
                                for eid in channel_eids:
                                    ...
                                    if tokens > 0:
                                        expert_layer = copy.deepcopy(layer)
                                        expert_layer.m = tokens
                                        print(f'Channel {channel_id} Expert {eid} acc tokens: {tokens}')
                                        t, e = self.devices['Acc'].get_time_and_energy(expert_layer, batch_size)
                                        channel_time += t
                                        acc_total_energy = [a + b for a, b in zip(acc_total_energy, e)]
                                channel_exec_times[channel_id] = channel_time
                            acc_total_time = max(channel_exec_times.values()) if channel_exec_times else 0.0
                            print(f'acc channel exec times: {channel_exec_times}')
                        else:
    ...
    def expert_schedule_simulation_pimoe(self, layer_idx=0, batch_size=16, lin=1024):
        ...
        # 预计算硬件常量
        gpu_flops = self.devices['GPU'].peak_flops * self.devices['GPU'].num_xpu
        acc_flops = self.devices['Acc'].peak_flops * self.devices['Acc'].num_attacc / self.devices['Acc'].num_hbm / 2  # 每个channel的算力
        gpu_bw = self.devices['GPU'].peak_memory_bandwidth * self.devices['GPU'].num_xpu
        ...
        for key, stat in fusion_stats.items():
            ...
            # GPU时间分解：已在GPU上的专家不需要额外搬运权重。
            gpu_compute = (orig * flop_factor) / gpu_flops
            gpu_mem = weight_load_gpu + (orig * input_size_factor) / gpu_bw
            gpu_load = 0 if on_gpu else weight_transfer
            gpu_time = gpu_compute + gpu_mem + gpu_load
        ...
        # 按序号排序专家ID
        all_expert_ids.sort()
        initial_gpu_expert_ids = sorted(eid for eid in all_expert_ids if eid in expert_locs_set)
        initial_acc_expert_ids = [eid for eid in all_expert_ids if eid not in expert_locs_set]
        num_experts = len(initial_acc_expert_ids)
        ...
        for ch in range(num_channels):
            ...
            for _ in range(count):
                if idx < num_experts:
                    channel_experts[ch].add(initial_acc_expert_ids[idx])
                    idx += 1

        # 初始状态：尊重expert_locs_set，已在GPU上的专家直接驻留在GPU。
        gpu_expert_set = set(initial_gpu_expert_ids)
        ...
        while improved:
            ...
            if max_token_id is not None:
                ...
                if max(new_gpu_time, new_max_channel_latency) < max(gpu_total_time, max_channel_latency) or new_gpu_time <= new_max_channel_latency:
                    # 执行移动
        ...
        return {
            ...
            'channel_experts': {ch: sorted(channel_experts[ch]) for ch in channel_experts}
        }
    ...
    def expert_schedule_simulation_fused_token(self, layer_idx=0, batch_size=16, lin=1024):
        ...
        返回：
            ...
            以及延迟构成：gpu_latency_breakdown, acc_latency_breakdown
        """
        if self.hetero_name is DeviceType.NONE:
            token_threshold = 1
        else:
            token_threshold = self.get_expert_token_threshold()
        if token_threshold is None:
