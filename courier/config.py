...
def make_xpu_config(gpu_type: GPUType,
    ...
    if gpu_type == GPUType.A100a:
        ...
        config['GPU']["INTERFACE_BW"] = 128 * 1000 * 1000 * 1000 * 0.65 # Effective BW is around 65% of theoretical peak BW
        config['CPU']["ENERGY_TABLE"] = ENERGY_TABLE['CPU']

    elif gpu_type == GPUType.RTX4090:
        # Ref: RTX4090 whitepaper
        config['GPU']["NUM_CORE"] = 144
        config['GPU']["FLOPS_PER_DEVICE"] = 330 * 1000 * 1000 * 1000 * 1000 * 0.65
...
def make_model_config(name, dtype, moe=False):
    if not moe:
        ...
        moe_model_table = {}
        moe_model_table['DeepSeek-16B'] = [28, 2048, 16, 128, 0.6875, 1, 64, 6, 2]
